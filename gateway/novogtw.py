...
INTERVAL_PING = int(env.INTERVAL_PING)
    ...
    while True:
        try:
            ...
            if data_json["TIPO"] != "GTW":                
                # Verificar se o dispositivo já foi adicionado à lista de dispositivos
                device_type = data_json.get("TIPO")
                ...
                if achou == 0:
                    ...
                    print(f"Novo dispositivo {data_json['TIPO']} encontrado via multicast")
    ...
    while True:
        ...
        for dev in devices:
            ...
            try:
                ...
                print(f"Mensagem: {message} enviada para dispositivo {device_type}")

            except (socket.timeout, socket.error) as e:
                # Remove o dispositivo da lista de dispositivos caso ocorra erro de conexão
                devices[:] = [dev for dev in devices if not dev['TIPO'] == device_type]
                print(f"Falha no ping com {device_type}, dispositivo removido.")
                tcp_socket.close()
...
def change_device_state(device_type,device_ip, device_port, state):
    try:
        # Cria o socket TCP
        tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        tcp_socket.settimeout(10)  # Timeout para evitar bloqueios infinitos
        ...
        devices[:] = [dev for dev in devices if not dev['TIPO'] == device_type]
        print(f"Falha na conexão TCP com {device_type}, dispositivo removido.")
        tcp_socket.close()
...
def main():
    ...
    # Pingar dispositivos da lista via TCP
    ping_sensor_thread = threading.Thread(target=ping_devices)
    ping_sensor_thread.daemon = True
    ...
    # Muda estado de dispositivos atuadores através de conexão TCP
    while True:

        for dev in devices:
           if "SENSOR" not in dev['TIPO']:
            device_type = dev.get("TIPO")
            device_ip = dev.get("IP")
            device_port = dev.get("PORTA ENVIO TCP")

            change_device_state(device_type,device_ip, device_port, "0")
        ...
        for dev in devices:
           if "SENSOR" not in dev['TIPO']:
            ...
            change_device_state(device_type,device_ip, device_port, "1")
